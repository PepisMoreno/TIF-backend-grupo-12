...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:root@localhost/plantas2"
...
class Planta(db.Model):  # Producto hereda de db.Model
    ...
    riego = db.Column(db.String(100))
    suelo = db.Column(db.String(100))
    ubicacion = db.Column(db.String(100))
    imagen = db.Column(db.String(400))

    def __init__(self, nombre_comun, nombre_cientifico, luz, riego, suelo, ubicacion, imagen):
        """
...
class PlantaSchema(ma.Schema):
    ...
    """
    class Meta:
        fields = ("id", "nombre_común", "nombre_cientifico", "luz", "riego", "suelo", "ubicacion", "imagen")
...
def get_planta(id):
    ...
    Retorna un JSON con la información del producto correspondiente al ID proporcionado.
    """
    producto = Planta.query.get(id)  # Obtiene el producto correspondiente al ID recibido
...
def create_planta():
    ...
    new_planta = Planta(nombre_comun,nombre_cientifico, luz, riego, suelo, ubicacion, imagen)  # Crea un nuevo objeto Producto con los datos proporcionados
